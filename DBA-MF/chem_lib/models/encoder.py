...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        return self.propagate(edge_index, x=x, edge_attr=edge_embeddings),edge_index,edge_embeddings
...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        for layer in range(self.num_layer):
            h,index,edge = self.gnns[layer](h_list[layer], edge_index, edge_attr)
            if self.use_batch_norm:
        ...
        # h_stack = torch.stack(h_list[1:], dim=0)  # 不含输入x
        # node_representation = h_stack.mean(dim=0)
        return node_representation,x,index,edge
        # return node_representation,x,index,edge_attr
...
class GNN_Encoder(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        node_representation,emb,index,edge = self.gnn(x, edge_index, edge_attr)
        # graph_representation = self.pool(node_representation, batch)

        return node_representation,emb,index,edge
